...
use sentinel_core::vault::fileblob::{
    open_file, open_passphrase, pack_bundle, seal_file, seal_passphrase, unpack_bundle,
    BundleEntry, FileMeta,
};
...
/// The committed passphrase-wrapped (SPAS) blob must keep unwrapping to the inner SFIL blob — the
/// desktop half of the transfer-passphrase interop guarantee (the Swift half is in VaultCryptoTests).
/// Production-cost Argon2, so `#[ignore]`d like the wrapped-key vector; CI runs it in release mode.
#[test]
#[ignore = "production-cost Argon2; CI runs it explicitly in release mode"]
fn committed_fixture_passphrase_unwraps() {
    let f = read_fixture();
    let fp = &f["file_passphrase"];
    let wrapped = base64::engine::general_purpose::STANDARD
        .decode(fp["wrapped_b64"].as_str().unwrap())
        .unwrap();
    let pass = fp["passphrase"].as_str().unwrap();
    let inner = open_passphrase(pass, &wrapped, Argon2Profile::Production)
        .expect("SPAS decode changed incompatibly");
    assert_eq!(
        base64::engine::general_purpose::STANDARD.encode(&inner),
        fp["inner_blob_b64"].as_str().unwrap()
    );
    // The inner blob is exactly the committed SFIL file-transfer blob.
    assert_eq!(
        base64::engine::general_purpose::STANDARD.encode(&inner),
        f["file_transfer"]["blob_b64"].as_str().unwrap()
    );
    // Wrong passphrase must fail.
    assert!(open_passphrase("not the password", &wrapped, Argon2Profile::Production).is_err());
}
...
fn generate() {
    ...
    // A passphrase-wrapped (SPAS) golden vector over the SFIL blob above — Swift `openPassphrase`
    // must reproduce the inner blob. Argon2 cost is fixed at PRODUCTION (so this vector, like the
    // wrapped key, only round-trips when the Swift side derives at production cost too).
    let file_pass_passphrase = "correct horse battery staple";
    let file_pass_blob =
        seal_passphrase(file_pass_passphrase, &file_blob, Argon2Profile::Production);
    ...
    let fixture = serde_json::json!({
        ...
        "file_passphrase": {
            "passphrase": file_pass_passphrase,
            "inner_blob_b64": b64(&file_blob),
            "wrapped_b64": b64(&file_pass_blob),
        },
    });
    let out = concat!(
        env!("CARGO_MANIFEST_DIR"),
