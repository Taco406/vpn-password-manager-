...
### Fixed
- **The app no longer gets stuck trying to update itself.** It used to try installing an update
  every single time it opened, and on Windows that could leave it closing and reopening the same
  version forever. Now it just tells you an update is ready and installs it when you choose to,
  from **Settings → Updates**.
- **Your server tokens now actually reach your other devices.** Sharing your settings could fail
  silently — the app said "Shared" even when nothing left the computer — which is why a Hetzner
  token could show as "Connected" on the computer while your phone never saw those servers.
  **Account & Sync → Push to all my devices** now tells you the truth: what was shared, or the
  real reason it couldn't.

### Added
- **A Settings tab on iPhone and iPad.** Sync now (with a plain-English result), when you last
  synced, exactly which settings have arrived from your computer, Face ID, your server address,
  and disconnect — all in one place instead of buried in a menu.
